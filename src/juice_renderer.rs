...
use crate::{
    events::ModifyVisualizationEvent,
    simulation::{SimConstraints, SimDrain, SimFaucet, SimGrid, SimGridCellType, SimParticle},
    ui::{SimTool, UIStateManager},
    util::{
        self, cartesian_to_polar, degrees_to_radians, get_cursor_position, JUICE_BLUE, JUICE_GREEN,
        JUICE_SKY_BLUE,
    },
};
use bevy::{core_pipeline::prelude::ClearColor, prelude::*};
...
pub fn link_particle_sprite(
    ...
    particle_sprite_bundle.transform.translation = Vec3 {
        x: position.x,
        y: position.y,
        z: 0.0,
    };
    particle_sprite_bundle.transform.scale = Vec3 {
        x: 1.5,
        y: 1.5,
        z: 1.0,
    };
...
pub fn link_faucet_sprite(
    ...
    faucet_sprite_bundle.transform.translation = Vec3 {
        x: position.x,
        y: position.y,
        z: 1.0,
    };
    faucet_sprite_bundle.transform.rotation = Quat::from_rotation_x(PI);
    faucet_sprite_bundle.transform.scale = Vec3 {
        x: 0.01,
        y: 0.01,
        z: 1.0,
    };
...
pub fn link_drain_sprite(
    ...
    drain_sprite_bundle.transform.translation = Vec3 {
        x: position.x,
        y: position.y,
        z: 1.0,
    };
    drain_sprite_bundle.transform.rotation = Quat::from_rotation_x(PI);
    drain_sprite_bundle.transform.scale = Vec3 {
        x: 0.01,
        y: 0.01,
        z: 1.0,
    };
...
fn update_particle_color(
    ...
    match particle_render_data.color_render_type {
        ...
        FluidColorRenderType::Arbitrary => {
            color_particles(particles, particle_render_data.fluid_colors[0])
        }
        FluidColorRenderType::GridCell => {
            color_particles_by_grid_cell(particles, grid.as_ref(), JUICE_BLUE, JUICE_GREEN)
        }
    }
}
...
fn draw_solid_cell(grid: &SimGrid, cell_coordinates: Vec2, color: Color, gizmos: &mut Gizmos) {
    ...
    // Draw the cell.
    gizmos.rect_2d(position, 0.0, Vec2::splat(grid.cell_size as f32), color);
}
...
fn draw_grid_cells(grid: Res<SimGrid>, grid_render_data: Res<GridRenderData>, mut gizmos: Gizmos) {
    ...
    if !grid_render_data.draw_grid {
        let top_left: Vec2 = Vec2 {
            x: 0.0,
            y: grid_height,
        };
        let top_right: Vec2 = Vec2 {
            x: grid_width,
            y: grid_height,
        };
        let bottom_right: Vec2 = Vec2 {
            x: grid_width,
            y: 0.0,
        };
    ...
    for i in 0..((grid.dimensions.1 as u16) + 1) {
        ...
        gizmos.line_2d(
            cell_bottom_position,
            cell_top_position,
            grid_render_data.grid_color,
        );
    }
...
fn draw_tool_guides(
    ...
    match ui_state.selected_tool {
        SimTool::Grab => draw_selection_circle(
            &mut gizmos,
            cursor_position,
        ...
        SimTool::AddDrain => draw_selection_circle(
            &mut gizmos,
            cursor_position,
            ui_state.drain_radius,
            Color::GOLD,
        ),
        SimTool::AddWall => draw_selection_circle(
            &mut gizmos,
            cursor_position,
            grid.cell_size as f32 * 1.5,
            Color::GOLD,
        ),
        SimTool::RemoveWall => draw_selection_circle(
            &mut gizmos,
            cursor_position,
            grid.cell_size as f32 * 1.5,
            Color::SALMON,
        ),
        SimTool::AddFluid => draw_selection_circle(
            &mut gizmos,
            cursor_position,
            ui_state.add_remove_fluid_radius,
            Color::SEA_GREEN,
        ),
        SimTool::RemoveFluid => draw_selection_circle(
            &mut gizmos,
            cursor_position,
            ui_state.add_remove_fluid_radius,
            Color::ORANGE_RED,
