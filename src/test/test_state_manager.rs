...
use crate::simulation::sim_state_manager::{delete_particle, select_particles};
use crate::simulation::step_simulation_once;
#[cfg(test)]
use crate::simulation::{self, SimSurfaceDirection};
use crate::simulation::{
    sim_state_manager::{add_particle, add_particles_in_radius},
...
pub fn construct_new_simulation(
    ...
    for i in 0..50 {
        let _ = grid.set_grid_cell_type(49, i, SimGridCellType::Solid);
        let _ = grid.set_grid_cell_type(0, i, SimGridCellType::Solid);
        let _ = grid.set_grid_cell_type(i, 0, SimGridCellType::Solid);
        let _ = grid.set_grid_cell_type(i, 49, SimGridCellType::Solid);
    }
    ...
    println!(
        "Constructing the new simulation file with {} particles...",
        constraints.particle_count
    );
}
...
pub fn construct_test_simulation_layout(
    ...
    let _moar_test_particles = add_particles_in_radius(
        ...
        Vec2 {
            x: grid_center[0] * 0.7,
            y: grid_center[1] * 0.85,
        },
        Vec2::ZERO,
    );

    println!(
        "Creating a test simulation with {} particles...",
        constraints.particle_count
    );
}
...
pub fn construct_simulation_bias_test(
    ...
    let _test_particle_sphere = add_particles_in_radius(
        commands,
        constraints,
        grid,
        1.75,
    ...
    println!(
        "Creating a bias test simulation with {} particles...",
        constraints.particle_count
    );
}
...
pub fn test_select_particles(
    ...
    // Test particle selection.
    let selected_particles: Vec<Entity> =
        select_particles(particles, grid, cursor_position, radius);
...
#[cfg(test)]
fn test_add_faucet_update(mut commands: Commands, mut grid: ResMut<SimGrid>) {
    let faucet_pos = Vec2::new(grid.cell_size as f32, grid.cell_size as f32 * 10.0);
    let surface_direction = None;

    let Err(e) = simulation::sim_state_manager::add_faucet(
        &mut commands,
        grid.as_mut(),
        faucet_pos,
        surface_direction,
...
/// runs the add_faucet() function for testing.
#[cfg(test)]
fn test_add_drain_update(mut commands: Commands, mut grid: ResMut<SimGrid>) {
    ...
    let Err(e) = simulation::sim_state_manager::add_drain(
        &mut commands,
        grid.as_mut(),
        drain_pos,
        surface_direction,
...
fn run_faucet_test() {
    ...
    let before_count = juicebox_test
        .world
        .resource::<SimConstraints>()
        .particle_count;

    juicebox_test.update();

    // Get particle count after faucet has ran
    let after_count = juicebox_test
        .world
        .resource::<SimConstraints>()
        .particle_count;
